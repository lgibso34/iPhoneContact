	    ...
	    if(filesPresent.length==0){
	        ...
	        for(String fileName : filesPresent){  // looping through files in the directory
	            if(fileName.toLowerCase().endsWith(".vcf") && (new File(fileName).isFile())){
	        		...
	        		//System.out.println(last); //show last name
	        		
	        		output = "N;CHARSET=UTF-8:" + last + ";" + first + ";;;"; //write out line with proper values in their spots
	            	...
	            	if(!first.equals(""))
	                {out2 = new PrintWriter("C:\\Users\\Logan\\OneDrive\\ContactPJ\\contact iPhone\\new contacts\\" + first + " " + last + ".vcf");
	                //while(inFile2.hasNextLine())
	                //{
	                	throwaway = inFile2.nextLine();
	                	if(throwaway.equals("BEGIN:VCARD"))
	                	{
	                		while(!throwaway.equals("END:VCARD"))
	                		{
